...
const create = () => {
    ...
    const UpdateData = (e) => {
        ...
        console.log(data);
        axios.put('http://127.0.0.1:8000/api/post/update/'+id, data).then(res => {
            go('/');
        }).catch(err => {
    ...
    const getData = () => {
        axios.get('http://127.0.0.1:8000/api/post/edit/' + id).then(res => {
            console.log(res);
            setJudul(res.data.post.judul);
            setDeskripsi(res.data.post.deksripsi);
        }).catch(err => {
  ...
  <form onSubmit={UpdateData}>
    <div className="form-group mb-6">
      <input type="text" className="form-control block
        ...
        ease-in-out
        m-0
        focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" id="exampleInput7" value={getJudul}
